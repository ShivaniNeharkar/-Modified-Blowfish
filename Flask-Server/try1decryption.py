...
def swap_Function(p,q):
    swapper = p
    p = q
    q = swapper
    return p,q

def decryptDriver(keyy):
        for i in range(0,18):
                p[i] = p[i]^key[i%14]
        k = 0
        data = 0
...
def func(L):
    t_variable = s[0][L >> 24]
    t_variable = (t_variable + s[1][L >> 16 & 0xff]) % 2**32
    t_variable = t_variable ^ s[2][L >> 8 & 0xff]
    t_variable = (t_variable + s[3][L & 0xff]) % 2**32
    return t_variable
